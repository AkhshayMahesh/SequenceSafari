...
function game() {
    if (isCollide()) {
        if (k == 0) {
            ...
            localStorage.setItem("p1n",player)
            inits.play()
            document.getElementById("land2").showModal()
        } else if (k==1) {
            move.x=0
            move.y=0
            window.cancelAnimationFrame(myReq)
            ...
            if(p1>score){
                document.getElementById("won").innerHTML=`<br>${p1n} won with ${p1} points!!<br>`
            }else if (score>p1){
                document.getElementById("won").innerHTML=`<br>${player} won with ${score} points!!<br>`
            }else{
                document.getElementById("won").innerHTML=`<br>Hurray!! ${p1n} and ${player}, You scored the same!<br>`
            }
